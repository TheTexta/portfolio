 ...
 *  - Shaders available at '/components/projects/nepobabiesruntheunderground/preview/assets/shaders/tvstatic.vert' and '/components/projects/nepobabiesruntheunderground/preview/assets/shaders/tvstatic.frag'
 *
...
const SHADER_BASE_URL =
  "/components/projects/nepobabiesruntheunderground/preview/assets/shaders";
...
class TVStaticApp {
    constructor(options = {}) {
        ...
        const DEFAULTS = {
            ...
            mouseFollower: false,
            filterBrightness1: 1.0,
            filterContrast: 1.0,
            filterBrightness2: 1.0,
            filterInvert: false
        };
    ...
    async loadShaders() {
        const vertexShaderSource = await this.fetchShader(`${SHADER_BASE_URL}/tvstatic.vert`);
        const fragmentShaderSource = await this.fetchShader(`${SHADER_BASE_URL}/tvstatic.frag`);
        ...
        this.uniforms = {
            ...
            filterBrightness1: this.gl.getUniformLocation(this.program, 'u_filterBrightness1'),
            filterContrast: this.gl.getUniformLocation(this.program, 'u_filterContrast'),
            filterBrightness2: this.gl.getUniformLocation(this.program, 'u_filterBrightness2'),
            filterInvert: this.gl.getUniformLocation(this.program, 'u_filterInvert')
        };
    ...
    startRenderLoop() {
        const render = (timestamp) => {
            ...
            if (this.program) {
                ...
                this.gl.uniform1f(this.uniforms.filterBrightness1, this.config.filterBrightness1);
                this.gl.uniform1f(this.uniforms.filterContrast, this.config.filterContrast);
                this.gl.uniform1f(this.uniforms.filterBrightness2, this.config.filterBrightness2);
                this.gl.uniform1i(this.uniforms.filterInvert, this.config.filterInvert ? 1 : 0);
            }
...
window.createTVStaticOverlay = function(canvasId = 'tv-static-canvas', options = {}) {
    ...
    const filterProfileByCanvasId = {
        'tv-static-top': {
            filterBrightness1: 0.5,
            filterContrast: 2.0,
            filterBrightness2: 2.0,
            filterInvert: false
        },
        'tv-static-tracking-cursor-pos': {
            filterBrightness1: 0.5,
            filterContrast: 4.0,
            filterBrightness2: 4.5,
            filterInvert: true
        }
    };
    const filterProfile = filterProfileByCanvasId[canvasId] ?? {};
