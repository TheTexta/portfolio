...
uniform float u_filterBrightness1;
uniform float u_filterContrast;
uniform float u_filterBrightness2;
uniform int u_filterInvert;
...
void main() {
    ...
    color *= max(0.0, u_filterBrightness1);
    color = (color - 0.5) * u_filterContrast + 0.5;
    color *= max(0.0, u_filterBrightness2);
    if (u_filterInvert == 1) {
