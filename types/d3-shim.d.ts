declare module "d3" {
  ...
  > {
    force(name: "link"): ForceLink<NodeDatum, LinkDatum> | undefined;
  ...
  export function select<GElement extends EventTarget>(
    node: GElement,
  ): Selection<GElement>;

  export interface D3ZoomEvent<GElement extends EventTarget, Datum> {
    transform: ZoomTransform;
    sourceEvent: Event & { currentTarget?: GElement | null };
    type: string;
    datum?: Datum;
  }
  ...
  export interface ZoomBehavior<GElement extends EventTarget, Datum> {
    ...
    filter<EventType = unknown>(filterFn: (event: EventType) => boolean): this;
    on(
      type: string,
  ...
  export function zoom<GElement extends EventTarget, Datum>(): ZoomBehavior<
    GElement,
    Datum
  >;
  ...
  export interface DragBehavior<GElement extends EventTarget, Datum, Subject> {
    ...
    subject<EventType = unknown>(
      subjectFn: (event: EventType, datum: Datum) => Subject | null,
    ): this;
    on(
      type: string,
      listener: (event: D3DragEvent<GElement, Datum, Subject>) => void,
    ): this;
  }
  ...
  export function pointer<EventType = unknown>(
    event: EventType,
    target?: EventTarget,
  ): [number, number];
}
